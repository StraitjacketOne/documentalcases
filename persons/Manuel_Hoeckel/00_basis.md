# Manuel Hoeckel - Basis

**Erstellt:** 20.12.2025
**Status:** Dokumentiert
**Spitznamen:** "Pausenclown", "Schnecki" (angeblich von Mark - BESTRITTEN), "Schnetti der Clown"

---

## Der Name "Sunny Schnett" / "Sahn ich nett"

**Telefonat Brand 08.09.2023:**
> "Ich frage den Herr Hoeckel im Gespraech, weil er mir die ganze Zeit immer ans Wort gefallen ist, **wie ist Ihr Name?** Und was kriege ich gesagt? **Sanisnet, Sanisnet, Sanisnet**, ist das normales Verhalten?"

**Brief an Hellenthal 09.01.2024:**
> "Auf die Frage wie denn eigentlich sein Name sei, wurde mir gesagt: **'Sahn ich nett'** — Zuerst dachte ich noch er hiesse **'Sunny Schnett'**"

**Marks Erklaerung:** Weil Hoeckel "ein Clownsartiges Gehabe an den Tag legte", behielt Mark ihn "fuer alle Zeit als **'Schnetti der Clown'** in Erinnerung" — NICHT weil er ihn "Schnecki" nannte.

---

## Grunddaten

| Feld | Wert |
|------|------|
| Name | Manuel Hoeckel |
| Funktion | Mitarbeiter Kinderschutzteam |
| Behoerde | Jugendamt Regionalverband Saarbruecken |
| Abteilung | Sozialer Dienst, Abteilung 51.2 |
| Vorgesetzter | Heiko Bluth (Sachgebietsleiter) |

---

## Rolle im Fall

### Kernvorwurf: Beweisunterdrueckung durch "Hunger" (14.09.2022)

**Was Mark berichtet (Tagebuch 12.09.2023):**

> "Jedoch raeumte man mir keine Gelegenheit mehr ein diese abzuspielen, weil ein Herr Hoeckel, der bei diesem Gespraech ebenfalls dabei war, kundtat **dass er Hunger hatte** und das Gespraech somit einseitig beendet wurde."

> "Mein Gesuch dieses Gespraech zu einem spaeteren Zeitpunkt an diesem Punkt wieder aufzugreifen und fortzufuehren wurde zwar muendlich bejaht, jedoch kam es in dieser Konstellation nie..."

**Konsequenz:** Mark konnte seine Beweise (Audioaufnahmen der alkoholisierten Kindesmutter) NICHT vorlegen.

---

## Chronologie der Vorfaelle

### 14.09.2022 - Das "Hunger"-Gespraech (Brief Hellenthal 09.01.2024)

**Ueberschrift im Brief:**
> "14.09.2022 — **Entmenschlichung, Vorfuehrung, Herablassung, Rechtsfreier Raum, Willkuer**"

| Aspekt | Detail |
|--------|--------|
| Anlass | Mark wollte Beweise vorlegen (Alkohol-Audios) |
| Anwesend | Hoeckel, Eichberger |
| Hoeckels Verhalten | "pausenlos provoziert", "immer ins Wort gefallen" |
| Abbruch | Hoeckel: "Hunger" |
| Folge | Beweise NIE vorgelegt |

**Hoeckels Verhalten (Brief Hellenthal):**

> "alles was ich sagte **mit rollenden Augen und mit spoettischen aufgesetzten Grinsen quittierte**"

> "immerzu sehnend den **Blick der Bestaetigung** 'das hier jetzt aber mal so richtig zum Lachen zu finden was dieser Vater da sagt' **Richtung Herrn Eichberger suchte**"

**Der "Pausenclown"-Moment:**

> "mit mittlerweile **traenendurchtraenkten Augen und bruechiger Stimme** mahnend sich an einen Herrn Eichberger wendet und sagt: 'Herr Eichberger was soll das denn bitte, wir hatten abgemacht, dass ich hier frei reden kann. Hier geht es um mein Kind. Und dauernd unterbrochen zu werden von hier so einem **Pausenclown** war nicht geplant'"

**Hoeckels Reaktion:**

> "Es dauerte etwa 3 Sekunden da **schlug er mit hochrotem Kopf mit der Faust auf den Tisch** und schrie: '**So! Sie haben mich beleidigt, sie fliegen raus!**'"

> "**Eichberger war genauso perplex wie ich** in dem Moment."

**Audio 24.09.2022 (Mark an JA-Bereitschaft):**

> [00:57] "Der Herr Hoeckel hat mich pausenlos provoziert."
> [01:00] "Der ist mir immer ins Wort gefallen. Andauernd."

### 14.09.2022 (nachmittags) - Eichberger entschuldigt sich

**Telefonat Brand 08.09.2023:**

> "Und am Schluss war der **Herr Eischberger genauso geschockt ueber das Verhalten von Herrn Hoecke** wie ich."

> "Dass der Herr Eischberger **nachtraeglich noch angerufen hat und sich entschuldigt hat** fuer den Tag, habe ich auch auf Sprache."

> "Ich fand es gut, dass er mich zurueckgerufen hat und **es nicht okay fand, was der Herr Hoecke gemacht hat** in dieser Besprechung."

---

### 05.10.2022 - "Schnecki"-Vorfall (BESTRITTEN)

**Was das JA behauptet (Bluth vor Gericht 25.10.2022):**

> "Das 'Schnecki' sei gefallen im Vorbeigehen des Mitarbeiters an Herrn Jaeckel in der Situation im Jugendamt am 05.10.2022"

**Was Mark sagt (Telefonat mit Kuhn 10.01.2023):**

> "Dann steht in dem komischen Schreiben da. Da steht drin, ich haette Schnecki gesagt. **Keine Sau fragt mich. Warum sagen Sie Schnecki?**"

**Was Mark an RA Lehne schreibt (11.05.2023):**

> "Ich habe naemlich **nie Schnecki gesagt** und genau das ist wiederum ein paradebeispiel fuer die Arbeit vom Jugendamt in dieser Abteilung."

---

### 04.09.2023 - Bereitschaftstelefon (Anklageschrift)

**Vorwurf (laut Anklageschrift vom 04.01.2024):**

> "Der Geschaedigte Manuel Hoeckel, der das Telefonat entgegennahm, sei von dem Angeschuldigten als Kinderschaender und mit der Aussage, der Geschaedigte habe sein Kind missbraucht, beschimpft worden."

**Angebliche Worte (Schreiben Berg 05.09.2023):**

> "Du bist doch der Poppers-Manuel! Stehst du auf Fisten? Du bist ein Kinderschaender! Du hast mein Kind geschaendet! Du wirst vor Gericht sehen, was du davon hast!"

**Kontext:** Nicolas wurde am Wochenende (02.09.2023) in Obhut genommen nachdem Mark Gefahr gemeldet hatte - aber NICHT zu Mark gegeben, obwohl ER die Meldung machte.

---

## Das Muster: Beweisunterdrueckung

### Die "Hunger"-Strategie

| Phase | Aktion |
|-------|--------|
| 1 | Mark bekommt Termin nach 3 Monaten Warten |
| 2 | Mark will Beweise vorlegen (Alkohol-Audios) |
| 3 | Hoeckel provoziert, faellt ins Wort |
| 4 | Hoeckel: "Hunger" - Gespraech beendet |
| 5 | Beweise WERDEN NIE GEHOERT |
| 6 | Mark wird als "querulatorisch" abgestempelt |

**Mark im Telefonat 08.09.2023 mit Brand:**

> "Wenn ich akute Gefahrenmeldungen machen will beim Jugendamt und diese Beweise einfach nicht auf den Tisch kommen, weil ein Herr Hoeckel das nicht zulaesst, ist das korrektes Verhalten von Jugendamt?"

---

## Gerichtliche Relevanz

### Zeuge im Hauptverfahren

Aus `extracted/Hauptverfahren.txt`:

> "Zeuge Manuel Hoeckel, Saarbruecken"

### Anklagepunkte gegen Mark (betreffend Hoeckel)

| Datum | Vorwurf |
|-------|---------|
| 04.09.2023 | Beleidigung ("Kinderschaender", "Poppers-Manuel") |
| 04.09.2023 | Falsche Verdaechtigung (Missbrauchsvorwurf) |

---

## Verbindungen

| Person | Beziehung |
|--------|-----------|
| Heiko Bluth | Vorgesetzter (berichtet ueber "Schnecki") |
| Adrian Stolz | Kollege im Kinderschutzteam |
| Alexander Eichberger | Erster Sachbearbeiter (gemeinsamer Termin) |
| Nina Meiser | Kollegin (erste Sachbearbeiterin) |

---

## Dokumente

| Datum | Dokument | Relevanz |
|-------|----------|----------|
| 14.09.2022 | Termin JA | "Hunger"-Abbruch, Faust auf Tisch |
| 24.09.2022 | Audio-Transkript | Provokation dokumentiert |
| 25.10.2022 | Gerichtsprotokoll | Bluth berichtet "Schnecki" |
| 10.01.2023 | Telefonat Kuhn | Mark bestreitet "Schnecki" |
| 11.05.2023 | Email an RA Lehne | "Nie Schnecki gesagt" |
| **08.09.2023** | **Telefonat Brand** | **"One-Man-Show", Eichberger entschuldigte sich** |
| 12.09.2023 | Tagebuch | "Hunger"-Vorfall dokumentiert |
| 05.09.2023 | Schreiben Berg | Beleidigungsvorwuerfe |
| 04.01.2024 | Anklageschrift | Hoeckel als Geschaedigter |
| **09.01.2024** | **Brief an Hellenthal** | **Vollstaendige Darstellung, "Sunny Schnett"** |

Pfade:
- `extracted/2022-09-24_RVS-JA-Bereitschaftsdienst_Telefonat_Kindeswohlgefaehrdung-Manipulation-48h-Falle.txt`
- `extracted/2022-10-25_AG-Saarbruecken_Hellenthal_Eroerterungstermin-Kindschaftssache_39F221-22.txt`
- `extracted/2023-01-10_RVS-JA_Kuhn_Telefonat-Umgangsregelung_39F221-22.txt`
- `extracted/2023-05-11_Jaeckel_RA-Lehne_Email-3-Fragen-Verfahrenskritik_17-23.txt`
- `extracted/2023-09-08_Jaeckel_JA-Brandt_Telefonat-Transkript-Nicolas-Geburtstag_Umgangsrecht.txt`
- `extracted/2023-09-12_Tagebuch_Auszuege.txt`
- `extracted/2024-01-09_Jaeckel_AG-Saarbruecken_Ausfuehrliche-Sachverhaltsdarstellung_Jugendamt-SB_FAX01.txt`
- `extracted/2024-01-09_Jaeckel_AG-Saarbruecken_Ausfuehrliche-Sachverhaltsdarstellung_Jugendamt-SB_FAX02.txt`
- `extracted/GAJaeckelMarkSiegfried_22072025.txt`

---

## Bewertung

### Die Funktion im System

Manuel Hoeckel hatte eine klare Aufgabe: **Verhindern, dass Marks Beweise gehoert werden.**

Die "Hunger"-Methode ist dabei besonders perfide:
- Kein direkter Konflikt
- Keine nachweisbare Ablehnung
- Einfach... abbrechen

### Das Ergebnis

| Was Mark wollte | Was passierte |
|-----------------|---------------|
| Alkohol-Beweise vorlegen | "Hunger" - Abbruch |
| Gehoert werden | "Pausenlos provoziert" |
| Gespraech fortsetzen | "Kam nie dazu" |

---

**Aktualisiert:** 20.12.2025
**Methodik:** Truthkeeper Dokumentenanalyse
